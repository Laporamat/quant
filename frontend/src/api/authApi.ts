...
export interface TokenResponse {
  access_token: string; token_type: string; expires_in: number; user: AuthUser
}
export interface Session {
  id: string; device_info: string; ip_address: string
  expires_at: string; created_at: string; last_used_at: string
}
...
export const authApi = {
  ...
  login: (login: string, password: string) =>
    http.post<TokenResponse>('/auth/login', { login, password }),

  logout: (refresh_token?: string) =>
    http.post('/auth/logout', { refresh_token }).catch(() => {}),
  ...
  updateProfile: (display_name: string) =>
    http.put<AuthUser>('/auth/me', { display_name }),

  changePassword: (current_password: string, new_password: string) =>
    http.post('/auth/change-password', { current_password, new_password }),

  sessions: () =>
    http.get<{ sessions: Session[] }>('/auth/sessions'),
  ...
  auditLog: (limit = 20) =>
    http.get<{ logs: Record<string, unknown>[] }>(`/auth/audit?limit=${limit}`),
}
